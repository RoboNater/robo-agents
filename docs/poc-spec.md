...
| `agent` | name, status (`idle`/`busy`/`released`/`lost`), context_id, worker_instance_id, last_heartbeat, last_progress_at, current_task_id, **profile:** harness, harness_version, provider, model, model_source (`declared`/`env`/`unknown`), declared_model, capabilities[], workspace_id? | registered on check-in; profile replaced wholesale on each check-in |
