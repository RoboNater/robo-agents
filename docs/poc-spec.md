...
- Assignment replay: every `assign_task` passes the triggering event's stable `id` as `event_id`, never its per-delivery `delivery_id`. An exact replay returns the task already created from that event even when the task has since completed; a changed payload is a conflict.
